"""Experiment 6: symmetric Fourier approximation of the open-system generator.

This experiment studies a symmetric temporal-Fourier truncation of a time-dependent
Lindblad/GKSL generator.
...
The level-N approximation keeps the symmetric |k| <= N band. We propagate the
partial-sum generator (re-rotating the envelope back to the lab frame) and report:

  * the mean state fidelity of the truncated trajectory to the exact one;
  * the sum of Frobenius norms of omitted superoperator coefficients, which
    upper-bounds the F-to-F generator-approximation error. The trace-norm
    trajectory bound has an additional sqrt(d) factor and this weight is not an
    infidelity bound.

The k=0 mode is a time average and is GKSL. Generic N>0 partial sums preserve
Hermiticity and trace but are not guaranteed to preserve complete positivity;
therefore the experiment is an approximation study, not a proof of physicality.
As N grows the partial sum converges to the sampled Fourier representation and the
fidelity approaches the reference trajectory. This is deterministic (no training,
no seeds).
"""
...
# Levels reported (retained symmetric temporal-frequency band).
DEFAULT_LEVELS = [0, 1, 2, 3, 4, 6, 8, 12, 16, 24]
_LABEL = {0: 'time average', 1: 'first symmetric band'}
...
def run(config: dict = None):
    ...
    with open(table_path, 'w') as fh:
        ...
        fh.write(r"\caption{Experiment 6: symmetric Fourier approximation of the "
                 ...
                 r"the omitted Frobenius coefficient weight "
                 r"$\sum_{|k|>N}\lVert \mathbb L_k\rVert_F$ that bounds the $F\!\to\!F$ "
                 r"generator-approximation error. $N{=}0$ is the time-averaged generator; "
                 r"generic finite $N>0$ partial sums are Hermiticity- and trace-preserving but "
                 r"are not guaranteed to be GKSL. Deterministic (no seeds).}" + "\n")
