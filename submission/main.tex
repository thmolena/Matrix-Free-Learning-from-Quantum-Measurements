...
% Single-column article-class format with numbered referencing, suitable for Nature's
% flexible initial-submission format. Compiles with a standard TeX distribution
% (TeX Live) or tectonic. Springer Nature also provides sn-jnl.cls if their template is required.
...
All numerical values in this section are produced by the scripts in \texttt{submission/code}, written directly to the figure and table files included with the manuscript; none are entered by hand. Simulations run on CPU using PyTorch and SciPy~\cite{paszke2019pytorch,virtanen2020scipy}, with Adam~\cite{kingma2015adam} for the physics-informed runs. The Lindblad residual derivative $\dot\rho$ is obtained by automatic differentiation of the density network with respect to time. The aim is to test the structural claims---exact physicality, parameter recovery, robustness under sparse observations, and compiler cost---on small, fully reproducible problems, not to claim production-scale or hardware performance. We report five experiments. A single-qubit inverse problem, a sparse-measurement ablation, and a dense-versus-structured compiler benchmark support the core claims directly. A two-qubit dissipative gate shows that the \emph{recovered generator} transfers across held-out initial states. A qutrit study isolates the spectral bias of a plain temporal network and shows that Fourier time-features substantially improve reconstruction; precise rate identification in that fast-oscillatory regime is left as future work (Sec.~\ref{sec:limitations}). Larger studies should use batched automatic differentiation, mixed precision where safe, and GPU-resident residual kernels.

\subsection{Experiment 1: single-qubit inverse problem}
...
\begin{equation}
    ...
    \quad \Omega=2\pi(0.1),
    \quad \gamma_1=0.30,
    \quad \gamma_\phi=0.15.
\end{equation}
The initial state is $\rho_0=\ket{+}\bra{+}$, and the observations are noisy samples of $\langle\sigma_x\rangle$, $\langle\sigma_y\rangle$, and $\langle\sigma_z\rangle$ on a uniform grid over $t\in[0,3]$ (100 points) with additive Gaussian noise of standard deviation $0.02$. We compare the CPTP-Cholesky network against a Hermitian trace-one baseline of the form $\rho(t)=\frac12(I+x(t)\sigma_x+y(t)\sigma_y+z(t)\sigma_z)$, which does not enforce positivity. Both models jointly fit the four generator parameters and the trajectory.

\input{tables/exp1_parameter_errors.tex}

Table~\ref{tab:exp1} reports the relative parameter-recovery errors. The CPTP-Cholesky network recovers all four parameters more accurately than the unconstrained baseline---for example $\omega$ to $0.1\%$ versus $1.0\%$, $\Omega$ to $1.9\%$ versus $8.6\%$, and the dephasing rate $\gamma_\phi$ to $8.7\%$ versus $48\%$---and attains a mean state fidelity of $1.000$ against the exact trajectory (baseline $0.9998$), while remaining positive semidefinite by construction at every time and every iteration. Figures~\ref{fig:exp1_fidelity} and~\ref{fig:exp1_recovery} show, respectively, the state fidelity over time and the reconstructed observables.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{exp1_state_fidelity.pdf}
    \caption{Single-qubit inverse problem: state fidelity between the learned and exact density-matrix trajectories. The hard-constrained CPTP-Cholesky model stays inside the density-matrix set by construction and tracks the exact trajectory to near-unit fidelity.}
    \label{fig:exp1_fidelity}
\end{figure}

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{exp1_parameter_recovery.pdf}
    \caption{Single-qubit inverse problem: measured observables and the learned reconstruction. Both models are trained from the same noisy data; the constrained model achieves lower parameter-recovery error (Table~\ref{tab:exp1}).}
    \label{fig:exp1_recovery}
\end{figure}

\subsection{Experiment 2: robustness under sparse measurements}
\label{sec:experiment_sparse}

Using the same qubit system, we randomly retain only a fraction of the observation entries and ask how trajectory recovery degrades. The CPTP-Cholesky network is compared against an unconstrained density network (\texttt{UnconstrainedDensityNet}) of matched width and depth, at observation fractions $1.0$, $0.5$, $0.25$, and $0.10$. We report the mean trace distance to the exact trajectory.

\input{tables/exp2_sparse_ablation.tex}

As shown in Table~\ref{tab:exp2} and Figure~\ref{fig:exp2}, the constrained model keeps the mean trace distance between $0.002$ and $0.011$ across all fractions---one to two orders of magnitude below the unconstrained baseline ($\approx 0.62$)---and degrades only mildly as observations are removed down to $10\%$. Architectural physicality is the decisive difference: the baseline can drift outside the positive cone, while the Cholesky map cannot.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{exp2_sparse_measurements.pdf}
    \caption{Sparse-measurement ablation. Mean trace distance to the exact trajectory as a function of the retained observation fraction, for the CPTP-Cholesky model and an unconstrained density-network baseline of matched size.}
    \label{fig:exp2}
\end{figure}

\subsection{Experiment 3: reconstructing fast qutrit (leakage) dynamics}
\label{sec:experiment_qutrit}

We next test a transmon-like three-level system with basis states $\ket{0},\ket{1},\ket{2}$, anharmonic Hamiltonian $H=E_1\proj{1}+E_2\proj{2}$ at $E_1=2\pi(2.0)$ and $E_2=2\pi(3.8)$, and cascade decay plus dephasing
\begin{equation}
    L_{10}=\sqrt{\gamma_{10}}\,\ket{0}\bra{1},\quad
    L_{21}=\sqrt{\gamma_{21}}\,\ket{1}\bra{2},\quad
    L_\phi=\sqrt{\gamma_\phi}\,\diag(0,1,2),
\end{equation}
...
The remedy is to encode time with a deterministic Fourier feature basis, $t\mapsto[\,t,\ \sin(k\omega_0 t),\ \cos(k\omega_0 t)\,]_{k=1}^{K}$ with fundamental $\omega_0=2\pi/T$, before the multilayer perceptron. As Table~\ref{tab:exp3} and Figure~\ref{fig:exp3} show, this lifts the mean state fidelity from $0.88$ (plain MLP) to $0.95$ (Fourier features) on the same data, isolating spectral bias as the cause and the time encoding as the fix; the Cholesky parameterization keeps every output a valid density matrix throughout. Identifying the individual dissipative rates in this fast-oscillatory regime is harder---the rates are a small signal relative to the coherent dynamics, in line with the sensitivity analysis of Sec.~\ref{sec:gauge_failure_modes}---and we leave precise rate recovery here to future work (Sec.~\ref{sec:limitations}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{exp3_qutrit_leakage.pdf}
    \caption{Reconstructing fast qutrit dynamics. Dots are noisy population samples; curves show the exact and reconstructed population dynamics for the Fourier-feature density network. Fourier time-features raise the mean state fidelity from $0.88$ (plain MLP) to $0.95$.}
    \label{fig:exp3}
\end{figure}

\subsection{Experiment 4: a two-qubit dissipative gate generalizes across initial states}
\label{sec:experiment_twoqubit}

The two-qubit experiment uses a time-dependent exchange coupling $J(t)=J_0\exp[-(t-1.5)^2/(2\cdot0.3^2)]$ with $J_0=2\pi(0.05)$, local detunings $\Delta_1=2\pi(5.0)$ and $\Delta_2=2\pi(4.8)$, and local amplitude damping and dephasing on each qubit; Fourier time-features handle the fast detuning dynamics. The model is trained on the $\ket{00}$ initial state. The scientifically meaningful test is whether the \emph{learned generator}---rather than the single trained trajectory---predicts held-out initial states, so we propagate the recovered Lindbladian from $\ket{01}$, $\ket{10}$, and $\ket{+0}$ and compare to the exact dynamics.

\input{tables/exp4_gate_results.tex}

The network reproduces the trained $\ket{00}$ trajectory at fidelity $0.99$, and the recovered generator generalizes: propagated to the three held-out initial states it attains mean fidelities $0.97$, $0.97$, and $0.98$ (average $0.98$; Table~\ref{tab:exp4}). This is the property that matters for device characterization---a transferable open-system model rather than a memorized curve---and it holds because physicality and CPTP structure are enforced by construction while the few generator parameters are shared across all initial states.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.7\textwidth]{exp4_gate_fidelity.pdf}
    \caption{Two-qubit dissipative gate. State fidelity for the trained $\ket{00}$ initial state and for three held-out initial states, the latter obtained by propagating the learned generator. The recovered open-system model transfers across initial conditions at $\approx0.98$ mean fidelity.}
    \label{fig:exp4}
\end{figure}

\subsection{Experiment 5: dense-versus-structured compiler scaling}
...
In the CPU benchmark (Table~\ref{tab:exp5}, Figure~\ref{fig:exp5}) the structured evaluation is faster than dense at every tested dimension---from $5.8\times$ at $d=2$ to $12.6\times$ at $d=8$---and uses substantially less memory as $d$ grows, while the two residuals agree to floating-point precision. This matches the complexity theorem of Sec.~\ref{sec:compiler_theory}: dense storage scales as $d^4$, whereas the structured representation stores only $H$, the $m$ jump operators, and $\rho$.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{exp5_compiler_scaling.pdf}
    \caption{Compiler scaling benchmark. Dense superoperator storage and residual-evaluation time versus the structured matrix-free evaluation. The structured path is faster at all tested dimensions and avoids materializing the $d^2\times d^2$ Liouvillian.}
    \label{fig:exp5}
\end{figure}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item \textbf{Modest statistical power.} Each experiment uses a fixed seed and a small number of repeats; the study is a reproducible proof of concept, not a powered benchmark over many seeds, noise levels, and baselines.
    ...
    \item \textbf{No state-of-the-art claim.} The method is not benchmarked against all modern quantum system-identification approaches; the results should be read as a principled proof of concept.
\end{enumerate}
...
\subsection{Single-qubit and sparse-measurement experiments}

The density network (\texttt{DensityMatrixNet}) is a four-layer multilayer perceptron of width $96$ with GELU activations, mapping $t$ to the entries of a lower-triangular complex Cholesky factor; the diagonal entries pass through softplus with a small floor so that the output is strictly positive definite. The unconstrained baseline (\texttt{UnconstrainedDensityNet}) uses a matched architecture but outputs the density-matrix entries directly with no positivity constraint. Training uses Adam at learning rate $10^{-3}$, loss weights $\lambda_{\rm data}=1$, $\lambda_{\rm phys}=0.5$, $\lambda_0=10$, and a $60$-point collocation grid over the simulation window $t\in[0,3]$. The Lindblad residual derivative $\dot\rho$ is obtained by automatic differentiation of the network output with respect to time, not by finite differences. The sparse-measurement ablation (Sec.~\ref{sec:experiment_sparse}) reuses this setup and masks a random fraction of the observation entries. Hyperparameters were chosen for reproducibility on CPU rather than optimality.

\subsection{Qutrit and two-qubit experiments: Fourier time-features}

For the multi-level experiments (Secs.~\ref{sec:experiment_qutrit}, \ref{sec:experiment_twoqubit}) the scalar time input is augmented with a deterministic Fourier feature basis $t\mapsto[\,t,\ \sin(k\omega_0 t),\ \cos(k\omega_0 t)\,]_{k=1}^{K}$ with fundamental $\omega_0=2\pi/T$ and $K$ a few tens of modes, which removes the spectral bias of a plain multilayer perceptron on the coherent (Bohr-frequency) oscillations. The qutrit observations are populations $\langle\proj{k}\rangle$ together with coherence quadratures $\ket{i}\bra{j}+\mathrm{h.c.}$ and $-\ii(\ket{i}\bra{j}-\mathrm{h.c.})$ at $2\%$ noise; the reconstruction comparison in Sec.~\ref{sec:experiment_qutrit} uses an otherwise identical density network with $K=0$ (plain MLP) and $K=48$ (Fourier). For the two-qubit gate, the network is trained on $\ket{00}$; the learned generator (known detunings, learned coupling and rates) is then read out, and held-out initial states are propagated with the dense Lindblad solver and compared to the exact dynamics. Precise identification of the qutrit rates in the fast-oscillatory regime is left to future work: a least-squares fit of the linear-in-rates dissipator to the \emph{exact} trajectory recovers the rates to better than $1\%$, so the open difficulty is the accuracy of the learned trajectory's time derivative under noise, not identifiability.

\subsection{Compiler benchmark}

Dense mode used the Kronecker representation in~\eqref{eq:dense_liouvillian}. Structured mode directly evaluated commutator and dissipator terms. The relative error between the two evaluations is
\begin{equation}
