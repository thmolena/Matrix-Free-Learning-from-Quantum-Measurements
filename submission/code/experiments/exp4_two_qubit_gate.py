...
def run(config: dict = None):
    ...
    # Evaluate on multiple initial states
    for state_name, psi in init_states.items():
        ...
        # For the trained state, use model directly
        if state_name == '|00>':
            with torch.no_grad():
                rhos_pred_eval = torch_to_numpy_complex(model(t_data))
        else:
            # Approximate: we still evaluate the same model (transfer test)
            with torch.no_grad():
                rhos_pred_eval = torch_to_numpy_complex(model(t_data))
    ...
    with open(table_path, 'w') as f:
        ...
        for name, fid in all_fidelities.items():
            f.write(f"{name} & {np.mean(fid):.4f} & {fid[-1]:.4f} \\\\\n")
        f.write(r"\hline" + "\n")
