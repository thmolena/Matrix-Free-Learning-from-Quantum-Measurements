"""Spectral-frame parameterization and operator-system spectral truncation.

This module implements the two ideas that lift *spectral truncation* -- the
operator-system construction of noncommutative geometry (Connes--van Suijlekom)
that underlies C*-algebraic kernel machines (Hashimoto et al., spectral
truncation kernels) -- from static kernels to the *dynamical* problem of learning
and approximating an open-system (Lindblad/GKSL) generator.
...
2. OPERATOR-SYSTEM SPECTRAL TRUNCATION OF THE GENERATOR.
   ...
   w0 = 2*pi/T. The level-N spectral truncation keeps the |k| <= N band -- a
   projection onto a finite operator system of slow eigen-operators. Retaining the
   k=0 mode and conjugate-symmetric pairs preserves Hermiticity and trace; the
   N=0 (secular) truncation yields a TIME-INDEPENDENT CPTP (Davies) generator.
   The truncation error is bounded by the out-of-band spectral weight and
   vanishes at full band, so structured/dense evaluation is recovered exactly.
...
# ---------------------------------------------------------------------------
# Interaction-picture generator and its operator-system spectral truncation.
# ---------------------------------------------------------------------------
...
def _truncate_modes(modes: np.ndarray, n_keep: int) -> np.ndarray:
    ...
    pairs, so the truncated generator stays Hermiticity- and trace-preserving.
    """
    n = modes.shape[0]
...
def spectral_truncation_error(norms: np.ndarray, n_keep: int) -> float:
    """Out-of-band spectral weight sum_{|k|>N} ||L_k|| (the certificate bound)."""
    n = len(norms)
    keep = np.zeros(n, dtype=bool)
    keep[0] = True
    for k in range(1, n_keep + 1):
...
def propagate_truncated(modes: np.ndarray, n_keep: int, E: np.ndarray, V: np.ndarray,
    ...
    solver, then rotates the envelope back to the lab frame.
    """
