...
Writes the figure PDFs into ``submission/code/figures`` (the manuscript's
``\\graphicspath``) and the LaTeX table files into ``submission/code/tables``.
...
config['figures_dir'] = str(CODE / 'figures')
config['tables_dir'] = str(CODE / 'tables')
Path(config['figures_dir']).mkdir(parents=True, exist_ok=True)
Path(config['tables_dir']).mkdir(parents=True, exist_ok=True)

from ctpcpinn.plotting import plot_schematic
plot_schematic(str(CODE / 'figures' / 'fig1_schematic.pdf'))
print("fig1 schematic done", flush=True)
